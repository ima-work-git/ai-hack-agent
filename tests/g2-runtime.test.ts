...
const smallView = (name: string): GlassesView => ({ ...view(name), textSize: 'small' })
...
describe('G2Runtime — REQ-001/005/008, T-01/02/06/10 (injected bridge, not hardware)', () => {
  ...
  it('serializes two small-text image tiles and rebuilds ordinary text if images are rejected', async () => {
    ...
    expect(await runtime.connect()).toBe(true)
    expect(vi.mocked(bridge.createStartUpPageContainer).mock.calls[0]![0].containerTotalNum).toBe(3)
    await runtime.render(smallView('initial'))
    const page = vi.mocked(bridge.rebuildPageContainer).mock.calls[0]![0]
    ...
    const rendered = runtime.render(smallView('small'))
    expect(bridge.updateImageRawData).toHaveBeenCalledTimes(1)
    const newerFooter = runtime.render({ ...smallView('small'), footer: '音声認識中' })
    ...
    expect(await runtime.render({ ...smallView('small'), footer: '音声認識中：最新の言葉' })).toBe(true)
    expect(bridge.updateImageRawData).toHaveBeenCalledTimes(2)
    ...
    expect(await runtime.render(smallView('fallback'))).toBe(true)
    expect(bridge.rebuildPageContainer).toHaveBeenCalledTimes(2)
    expect(vi.mocked(bridge.rebuildPageContainer).mock.calls[1]![0].containerTotalNum).toBe(3)
  ...
  it('does not reuse a bitmap cache after an interrupted pair or a background transition', async () => {
    ...
    await runtime.connect(); await runtime.render(smallView('original'))
    ...
    const interrupted = runtime.render(smallView('changed'))
    const restored = runtime.render(smallView('original'))
    first.resolve(ImageRawDataUpdateResult.success)
    ...
    expect(await runtime.render(smallView('original'))).toBe(true)
    expect(f.bridge.updateImageRawData).toHaveBeenCalledTimes(5)
    await runtime.dispose()
  })

  it('uses small images only for an explicit body, then restores native list text without stopping audio', async () => {
    const f = fakeBridge()
    f.bridge.updateImageRawData = vi.fn(async () => ImageRawDataUpdateResult.success)
    f.bridge.rebuildPageContainer = vi.fn(async () => true)
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true, renderBitmap: () => [new Uint8Array([1]), new Uint8Array([2])] })
    await runtime.connect(); await runtime.startAudio({ continuous: true })
    await runtime.render(view('人物一覧'))
    expect(f.bridge.updateImageRawData).not.toHaveBeenCalled()
    expect(f.bridge.rebuildPageContainer).not.toHaveBeenCalled()
    await runtime.render(smallView('質問'))
    expect(f.bridge.updateImageRawData).toHaveBeenCalledTimes(2)
    expect(vi.mocked(f.bridge.rebuildPageContainer).mock.calls[0]![0].textObject!.find(value => value.containerName === 'footer')!.content).toBe('音声認識中')
    await runtime.render(smallView('該当文'))
    expect(f.bridge.updateImageRawData).toHaveBeenCalledTimes(4)
    expect(f.bridge.rebuildPageContainer).toHaveBeenCalledOnce()
    await runtime.render(view('人物一覧'))
    expect(vi.mocked(f.bridge.rebuildPageContainer).mock.calls[1]![0].containerTotalNum).toBe(3)
    expect(vi.mocked(f.bridge.textContainerUpgrade).mock.calls.at(-2)![0].content).toBe('人物一覧-content')
    expect(f.bridge.audioControl).toHaveBeenCalledTimes(1)
    expect(runtime.status.state).toBe('recording')
    await runtime.dispose()
  })

  it('falls back for a body that cannot fit, retaining all native content and allowing a later small body', async () => {
    const f = fakeBridge()
    f.bridge.updateImageRawData = vi.fn(async () => ImageRawDataUpdateResult.success)
    f.bridge.rebuildPageContainer = vi.fn(async () => true)
    const long = '長い本文を一文字も削らない。'.repeat(70)
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true,
      renderBitmap: text => text === long ? null : [new Uint8Array([1]), new Uint8Array([2])] })
    await runtime.connect(); await runtime.render(smallView('small'))
    await runtime.render({ ...smallView('long'), content: long })
    expect(vi.mocked(f.bridge.textContainerUpgrade).mock.calls.some(([value]) => value.content === long)).toBe(true)
    expect(vi.mocked(f.bridge.rebuildPageContainer).mock.calls.at(-1)![0].containerTotalNum).toBe(3)
    await runtime.render(smallView('fits-again'))
    expect(vi.mocked(f.bridge.rebuildPageContainer).mock.calls.at(-1)![0].containerTotalNum).toBe(5)
    expect(f.bridge.updateImageRawData).toHaveBeenCalledTimes(4)
    await runtime.dispose()
  })

  it('keeps the complete body native when the bridge lacks image support', async () => {
    const f = fakeBridge()
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true })
    await runtime.connect()
    expect(await runtime.render(smallView('全文'))).toBe(true)
    expect(vi.mocked(f.bridge.textContainerUpgrade).mock.calls.some(([value]) => value.content === '全文-content')).toBe(true)
    await runtime.dispose()
  })

  it('rejects an old small body after its layout rebuild is superseded by a native list', async () => {
    const f = fakeBridge()
    f.bridge.updateImageRawData = vi.fn(async () => ImageRawDataUpdateResult.success)
    const rebuilt = deferred<boolean>()
    f.bridge.rebuildPageContainer = vi.fn(async () => true).mockImplementationOnce(() => rebuilt.promise)
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true, renderBitmap: () => [new Uint8Array([1]), new Uint8Array([2])] })
    await runtime.connect()
    const old = runtime.render(smallView('古い質問'))
    runtime.invalidateViews('new')
    const latest = runtime.render(view('人物一覧'), 'new')
    rebuilt.resolve(true)
    expect(await old).toBe(false); expect(await latest).toBe(true)
    expect(f.bridge.updateImageRawData).not.toHaveBeenCalled()
    expect(vi.mocked(f.bridge.textContainerUpgrade).mock.calls.some(([value]) => value.content?.includes('古い質問'))).toBe(false)
    expect(vi.mocked(f.bridge.rebuildPageContainer).mock.calls.at(-1)![0].containerTotalNum).toBe(3)
    await runtime.dispose()
  })

  it('does not complete an old image pair when an equal-content native view supersedes it', async () => {
    const f = fakeBridge()
    f.bridge.updateImageRawData = vi.fn(async () => ImageRawDataUpdateResult.success)
    f.bridge.rebuildPageContainer = vi.fn(async () => true)
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true, renderBitmap: () => [new Uint8Array([1]), new Uint8Array([2])] })
    await runtime.connect(); await runtime.render(smallView('initial'))
    vi.mocked(f.bridge.updateImageRawData).mockClear()
    const first = deferred<ImageRawDataUpdateResult>()
    vi.mocked(f.bridge.updateImageRawData).mockImplementationOnce(() => first.promise)
    const old = runtime.render(smallView('same'))
    const latest = runtime.render(view('same'))
    first.resolve(ImageRawDataUpdateResult.success)
    expect(await old).toBe(false); expect(await latest).toBe(true)
    expect(f.bridge.updateImageRawData).toHaveBeenCalledOnce()
    expect(vi.mocked(f.bridge.textContainerUpgrade).mock.calls.some(([value]) => value.content === 'same-content')).toBe(true)
    await runtime.dispose()
  })

  it('does not stop audio for a rejected obsolete layout while the user returns to the list', async () => {
    const f = fakeBridge()
    f.bridge.updateImageRawData = vi.fn(async () => ImageRawDataUpdateResult.success)
    const rebuilt = deferred<boolean>()
    f.bridge.rebuildPageContainer = vi.fn(async () => true).mockImplementationOnce(() => rebuilt.promise)
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true, renderBitmap: () => [new Uint8Array([1]), new Uint8Array([2])] })
    await runtime.connect(); await runtime.startAudio({ continuous: true })
    const old = runtime.render(smallView('質問'))
    const latest = runtime.render(view('人物一覧'))
    rebuilt.resolve(false)
    expect(await old).toBe(false); expect(await latest).toBe(true)
    expect(f.bridge.rebuildPageContainer).toHaveBeenCalledTimes(2)
    expect(vi.mocked(f.bridge.rebuildPageContainer).mock.calls[1]![0].containerTotalNum).toBe(3)
    expect(f.bridge.updateImageRawData).not.toHaveBeenCalled()
    expect(f.bridge.audioControl).toHaveBeenCalledTimes(1)
    expect(runtime.status.state).toBe('recording')
  ...
  it('stops rather than issuing a fallback after an uncancellable layout timeout', async () => {
    const f = fakeBridge()
    f.bridge.updateImageRawData = vi.fn(async () => ImageRawDataUpdateResult.success)
    const late = deferred<boolean>()
    f.bridge.rebuildPageContainer = vi.fn(() => late.promise)
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true, timeoutMs: 50, imageTimeoutMs: 50,
      renderBitmap: () => [new Uint8Array([1]), new Uint8Array([2])] })
    await runtime.connect(); await runtime.startAudio({ continuous: true })
    const old = runtime.render(smallView('slow'))
    await vi.advanceTimersByTimeAsync(50)
    expect(await old).toBe(false)
    expect(runtime.status).toEqual({ state: 'error', reason: 'display_timeout', recovery: 'reopen' })
    late.resolve(true); await Promise.resolve()
    expect(f.bridge.rebuildPageContainer).toHaveBeenCalledOnce()
    expect(f.bridge.updateImageRawData).not.toHaveBeenCalled()
    expect(f.bridge.audioControl).toHaveBeenCalledWith(false)
    expect(await runtime.render(view('later'))).toBe(false)
    await runtime.dispose()
  })

  it('keeps forwarding microphone audio while a layout and each serial image take longer than two seconds', async () => {
    const f = fakeBridge()
    const onAudio = vi.fn()
    f.bridge.rebuildPageContainer = vi.fn(() => new Promise<boolean>(resolve => { setTimeout(() => resolve(true), 3_000) }))
    f.bridge.updateImageRawData = vi.fn(() => new Promise<ImageRawDataUpdateResult>(resolve => { setTimeout(() => resolve(ImageRawDataUpdateResult.success), 3_000) }))
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true, onAudio,
      renderBitmap: () => [new Uint8Array([1]), new Uint8Array([2])] })
    await runtime.connect(); await runtime.startAudio({ continuous: true })
    const rendered = runtime.render(smallView('全文'))
    let complete = false; void rendered.then(() => { complete = true })
    for (let stage = 0; stage < 3; stage++) {
      await vi.advanceTimersByTimeAsync(2_500)
      expect(complete).toBe(false)
      expect(runtime.status).toEqual({ state: 'recording' })
      f.event(pcm())
      expect(onAudio).toHaveBeenCalledTimes(stage + 1)
  ...
  it('stops at the eight-second image deadline and blocks a late tile, queued view and capture restart', async () => {
    const f = fakeBridge()
    const first = deferred<ImageRawDataUpdateResult>()
    const onAudio = vi.fn(); const onStatus = vi.fn()
    f.bridge.rebuildPageContainer = vi.fn(async () => true)
    f.bridge.updateImageRawData = vi.fn(() => first.promise)
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true, onAudio, onStatus,
      renderBitmap: () => [new Uint8Array([1]), new Uint8Array([2])] })
    await runtime.connect(); await runtime.startAudio({ continuous: true })
    const rendered = runtime.render(smallView('遅い画像'))
    await vi.advanceTimersByTimeAsync(0)
    expect(f.bridge.updateImageRawData).toHaveBeenCalledOnce()
    await vi.advanceTimersByTimeAsync(7_999)
    expect(runtime.status).toEqual({ state: 'recording' })
    f.event(pcm()); expect(onAudio).toHaveBeenCalledOnce()
    const latest = runtime.render(view('新しい人物一覧'))
    await vi.advanceTimersByTimeAsync(1)
    expect(await rendered).toBe(false); expect(await latest).toBe(false)
    expect(runtime.status).toEqual({ state: 'error', reason: 'display_timeout', recovery: 'reopen' })
    expect(onStatus).toHaveBeenLastCalledWith({ state: 'error', reason: 'display_timeout', recovery: 'reopen' })
    expect(f.bridge.audioControl).toHaveBeenCalledWith(false)
    first.resolve(ImageRawDataUpdateResult.success); await vi.advanceTimersByTimeAsync(0)
    expect(f.bridge.updateImageRawData).toHaveBeenCalledOnce()
    expect(f.bridge.rebuildPageContainer).toHaveBeenCalledOnce()
    expect(f.bridge.textContainerUpgrade).not.toHaveBeenCalled()
    f.event(pcm()); expect(onAudio).toHaveBeenCalledOnce()
    expect(await runtime.connect()).toBe(false)
    expect(await runtime.startAudio({ continuous: true })).toBe(false)
    expect(await runtime.render(view('late'))).toBe(false)
    await runtime.dispose()
  })

  it('keeps the microphone acknowledgement deadline at two seconds with the longer image deadline', async () => {
    const f = fakeBridge()
    const opening = deferred<boolean>()
    const onAudio = vi.fn()
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true, imageTimeoutMs: 10_000, onAudio })
    await runtime.connect()
    vi.mocked(f.bridge.audioControl).mockReturnValueOnce(opening.promise)
    const started = runtime.startAudio({ continuous: true })
    await vi.advanceTimersByTimeAsync(1_999)
    expect(runtime.status).toEqual({ state: 'connected', reason: 'bridge_acknowledged' })
    await vi.advanceTimersByTimeAsync(1)
    expect(await started).toBe(false)
    expect(runtime.status).toEqual({ state: 'error', reason: 'audio_start_timeout', recovery: 'reopen' })
    expect(f.bridge.audioControl).toHaveBeenCalledWith(false)
    opening.resolve(true); await vi.advanceTimersByTimeAsync(0)
    f.event(pcm()); expect(onAudio).not.toHaveBeenCalled()
    expect(vi.mocked(f.bridge.audioControl).mock.calls.filter(([open]) => !open)).toHaveLength(2)
    await runtime.dispose()
  })

  it('caps an explicitly extended layout timeout at ten seconds', async () => {
    const f = fakeBridge()
    const late = deferred<boolean>()
    f.bridge.rebuildPageContainer = vi.fn(() => late.promise)
    f.bridge.updateImageRawData = vi.fn(async () => ImageRawDataUpdateResult.success)
    const runtime = new G2Runtime({ bridge: f.bridge, enableImageText: true, imageTimeoutMs: 60_000,
      renderBitmap: () => [new Uint8Array([1]), new Uint8Array([2])] })
    await runtime.connect()
    const rendered = runtime.render(smallView('bounded'))
    await vi.advanceTimersByTimeAsync(9_999)
    expect(runtime.status.state).toBe('connected')
    await vi.advanceTimersByTimeAsync(1)
    expect(await rendered).toBe(false)
    expect(runtime.status).toEqual({ state: 'error', reason: 'display_timeout', recovery: 'reopen' })
    late.resolve(true); await vi.advanceTimersByTimeAsync(0)
    expect(f.bridge.updateImageRawData).not.toHaveBeenCalled()
    await runtime.dispose()
  })
  ...
  it('times out a hanging bridge and never reports connection success', async () => {
    ...
    expect(runtime.status).toEqual({ state: 'error', reason: 'connect_timeout', recovery: 'reopen' })
  })
  ...
  it('surfaces a failed stop and rejects further capture instead of claiming the microphone stopped', async () => {
    ...
    expect(runtime.status).toEqual({ state: 'error', reason: 'audio_stop_failed', recovery: 'reopen' })
    f.event(pcm())
    expect(onAudio).not.toHaveBeenCalled()
    expect(await runtime.startAudio()).toBe(false)
    await runtime.dispose()
  ...
  it.each([false, true])('keeps active recording on duplicate foreground events (continuous=%s)', async continuous => {
    const f = fakeBridge(); const onAudio = vi.fn(); const onStatus = vi.fn()
    const runtime = new G2Runtime({ bridge: f.bridge, onAudio, onStatus })
    await runtime.connect(); await runtime.startAudio({ continuous })
    onStatus.mockClear()
    f.event({ sysEvent: { eventType: OsEventTypeList.FOREGROUND_ENTER_EVENT } })
    f.event({ textEvent: { eventType: OsEventTypeList.FOREGROUND_ENTER_EVENT } })
    f.event(pcm())
    expect(runtime.status.state).toBe('recording')
  ...
  it('does not downgrade a microphone start when an already-foreground ENTER arrives before or after its acknowledgement', async () => {
    const f = fakeBridge(); const onAudio = vi.fn(); const onStatus = vi.fn()
    const runtime = new G2Runtime({ bridge: f.bridge, onAudio, onStatus })
    await runtime.connect()
    const opening = deferred<boolean>()
    ...
    f.event({ textEvent: { eventType: OsEventTypeList.FOREGROUND_ENTER_EVENT } })
    expect(onStatus).toHaveBeenCalledExactlyOnceWith({ state: 'recording' })
    f.event(pcm()); expect(onAudio).toHaveBeenCalledOnce()
    expect(f.bridge.audioControl).toHaveBeenCalledExactlyOnceWith(true, AudioInputSource.Glasses)
    await runtime.dispose()
  })
  ...
  it.each([false, true])('stops in the background without automatic replay or recording (continuous=%s)', async continuous => {
    ...
    f.event({ sysEvent: { eventType: OsEventTypeList.FOREGROUND_ENTER_EVENT } })
    expect(runtime.status).toEqual({ state: 'connected', reason: 'resume_required' })
    expect(f.bridge.textContainerUpgrade).not.toHaveBeenCalled()
