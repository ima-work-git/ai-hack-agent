...
現在の確認経路は、UIとAPIを同じoriginで配信するサーバーをEvenアプリのローカルテストQRから開く方式です。スマホからlocalhostは開けません。端末から届くサーバーの`HOST`・`APP_ORIGIN`を設定し、初回は利用コードで認証します。QRにはアクセス先だけを含め、利用コード・APIキー・端末認証トークンを含めません。LANに開く場合も鍵や保存データの公開は禁止です。実APIを使う配信はHTTPSにしてください。
