...
入力から対象を計画 → 公開検索 → 本文取得 → 人物と根拠を検証 → 必要なら追加調査 → 最大4件の事実と質問を同時表示。スマホは2×2、G2は4行で、省略した原文と出典はスマホで確認できます。足りない枠は「未確認」とし、4件に埋めるための推測はしません。曖昧な人物は利用者へ確認し、本文やAPIの障害時は検証済みの情報へ縮退します。
