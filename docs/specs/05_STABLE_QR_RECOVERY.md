...
## 2026-09-23：公開監視のDNS回復

新しい公開URLがDNS直引きでは解決できてもOS resolverでは失敗する場合、通常fetchのDNS失敗だけを契機に、検証済みQuick Tunnelの`/api/status`を公開IPv4への接続固定と元hostnameのTLSで再確認する。全体10秒・16KiB・転送禁止・取消の制限を維持し、正常なトンネルの再発行ループを防ぐ。[Issue #32と検証記録](../validation/PUBLIC_DNS_RECOVERY.md)。
