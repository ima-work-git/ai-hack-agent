...
- 既定は端末内のみで起動します。OrcaRouter・Tavily・Xの実API疎通は確認済み。公開デプロイ・音声認識・G2実測・実費照合は未完了です。
