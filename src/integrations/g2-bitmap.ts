...
export function renderGlassesBitmap(content: string): [Uint8Array, Uint8Array] | null {
  try {
    ...
    const fontFamily = '"Hiragino Kaku Gothic ProN", "Noto Sans JP", sans-serif'
    ...
    const drawLine = (line: string, left: number, top: number) => {
      ...
      // New concise facts fit at 16px. Older long results retain every character;
      // shrink first, then constrain width instead of silently dropping the tail.
      if (width > 560) context.font = `${Math.max(10, Math.floor(16 * 560 / width))}px ${fontFamily}`
      context.fillText(text, left, top, 560)
    }
