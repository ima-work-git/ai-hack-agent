...
**現在の状態：MVPを実装しました。ブラウザーの架空データによる調査、根拠表示、同姓同名の確認、障害時の縮退を利用できます。OrcaRouter・Tavily・Xを使う調査で、出典付き2カードを返すことを確認しました。音声認識・Even G2実機は未検証です。提出完了ではありません。**
...
入力から対象を計画 → 公開検索 → 本文取得 → 人物と根拠を検証 → 必要なら追加調査 → 最大3枚のカード表示。曖昧な人物は利用者へ確認し、根拠不足は推測せず終了します。本文やAPIの障害時は検証済みの情報へ縮退します。

- **入力・表示**：手入力、同意後の最大30秒音声、スマートフォンの出典詳細、Even Hub SDK経由のG2表示。
- **制限**：調査20秒、個別操作8秒、LLM3回・検索2回・本文4件以内。実APIは鍵と予算が揃うまで停止。
