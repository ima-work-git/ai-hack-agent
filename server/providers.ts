...
export function createLiveProvider(config: ProviderConfig, dependencies: ProviderDependencies = {}): ResearchProvider {
  ...
  async function complete<T>(schema: z.ZodType<T>, instructions: string, data: unknown, signal: AbortSignal, timeoutMs = API_TIMEOUT_MS, finalReminder = '', modelOverride?: string): Promise<ProviderResult<T>> {
    requireConfigured(config.orcaApiKey, 'OrcaRouter APIキー');
    const model = modelOverride?.trim() || config.orcaModel;
    requireConfigured(model, 'OrcaRouterモデル');
    const response = checked(CompletionSchema, await apiJson(ORCA_COMPLETIONS, config.orcaApiKey, signal, {
      model, temperature: 0, max_tokens: 1800, stream: false,
  ...
  return {
    ...
    async assess(target: Target, sources: EvidenceSource[], signal): Promise<ProviderResult<Assessment>> {
      ...
      const questionInstructions = [
        'QUESTION QUALITY: Be a considerate conversation partner, not an interviewer, critic, examiner, or salesperson. Anchor every suggestedQuestion in a concrete activity, product, event, or idea in its selected fact. Ask for an answer the fact does not already give. Show sincere interest by naming one specific detail, then gently inviting the person to share one easy-to-answer aspect of their experience or perspective. A short interested acknowledgment is optional, never forced into every card.',
        'Use natural, polite Japanese that the wearer could actually say aloud (ですか／ましたか／でしょうか). Ask one question, not a list. Prefer an intended use case, a detail they care about, a memorable response, or a choice behind the concrete activity. Vary the angle across cards. Do not quiz them on definitions, ask them to defend a decision, demand failures or difficulties, or ask for a job/name/date already stated. Do not presume success, hardship, emotions, motives, or results that the selected fact does not establish. Do not demand improvements, feature strengthening, a business roadmap, strategy, or performance goals; this is a social conversation, not product coaching. For an app, invite its intended use case or a preferred interaction rather than asking どんな機能を強化したいですか, which presumes a need to improve. Asking what motivated a choice is different from inventing its motive.',
        'Avoid generic templates such as 投稿のきっかけは or 活動で工夫した点は, blunt interrogations such as なぜ失敗したのですか or 本当にご自身で作ったのですか, and unearned praise such as さすが or 成功の秘訣. Do not claim the wearer has read a book, used or loved a product, followed or been a fan of someone, or met the person before; no fabricated personal familiarity. No flattery, judgment, assumptions of a close relationship, or sensitive private-life topics. If a fact supports no worthwhile respectful question, omit that card; never fill its slot with a universal fallback.',
        'Examples of tone, not facts to copy: fact=音声学習アプリを公開した → その音声学習アプリ、特に試してほしい機能はありますか？; fact=木製の試作機を展示した → 木製の試作機、素材を選ぶときに大切にしたことは何ですか？; fact=翻訳の技術イベントで登壇した → 翻訳の登壇で、特に伝えたかったことは何ですか？. A bad follow-up to these facts is すごいですね、成功の秘訣は？ or どんな苦労を乗り越えましたか？ because it adds an unsupported evaluation or premise.',
        'A displayQuestion must preserve the same specific focus as its full question, its politeness, and its time perspective. For example full=その音声学習アプリ、特に試してほしい機能はありますか？ → display=音声学習アプリ、試してほしい機能はありますか？. Retain a concrete subject from the full question (a product, activity, technique, or named topic); never reduce it to その話題, その活動, それ, or どうでしたか. Do not replace a warm specific full question with a terse generic demand to fit. Omit the short helper when that meaning cannot fit within 26 characters; the full question remains available. Historical questions must retain the concrete focus and clearly acknowledge the past, such as 当時. Before returning, silently check each full/short pair for grounded detail, no invented premise, respectful tone, one answerable question, and no repeated angle.',
      ].join(' ') + ' ';
      const questionFinalReminder = [
        '最後に質問を必ず点検・書き直してください。目的は雑談で相手の具体的な活動に関心を向けることです。面接、改善指導、知識テストではありません。',
        '各質問には、選んだfactIdの本文に実際にある具体的な製品名・技術名・題材・活動の短い語句を少なくとも1つ残してください。「最近の投稿」「紹介した内容」「過去の投稿」「その話題」「COOとしての役割」は具体的な題材の代わりになりません。役職だけの事実から仕事の心構えを尋ねるカードを作らず、具体的な活動がある別のfactIdを選んでください。良い質問が作れなければカードを省略してください。',
        '本文に書かれたことをそのまま聞き返さず、本人が答えやすい小さな関心を1つだけ丁寧に尋ねてください。用途・気に入っている触り方・その選択で大切にした点などが候補です。成功や苦労や興味を決めつけず、改善点・機能強化・経営方針・ロードマップを要求しないでください。装着者が愛用・読書・面識・ファン歴を持つと捏造しないでください。',
        '例（資料にない固有名や経験はコピーしない）: 本文「音声学習アプリを公開」→「音声学習アプリ、どんな場面で使ってほしいですか？」。本文「木製の試作機を展示」→「木製の試作機、素材選びで大切にしたことは何ですか？」。本文「翻訳モデルの発表を紹介」→「翻訳モデルの発表で、気になった使い方はありますか？」。悪例「投稿で紹介した内容をどう活用してほしいですか？」「役割で大切にしていることは？」「どんな機能を強化したいですか？」は具体性または雑談の自然さが不足しています。',
        'displayQuestionも同じ具体的な題材・聞きたい点・丁寧さを保ってください。「その話題」に置き換えたり目的語を落としたりして26字に合わせないでください。保てない場合はdisplayQuestionを省略し、良い全文のsuggestedQuestionを残してください。すべての質問で上記を満たすまで返却前に修正してください。本人性と根拠の厳格な検証はそのまま維持してください。',
      ].join(' ');
      ...
      const result = await complete(SelectionAssessmentSchema,
        ...
        { target, identityAssessmentContext, verifiedIdentityAliases, sources: evidence }, signal, balanced ? 12_000 : API_TIMEOUT_MS, identityFinalReminder + questionFinalReminder + ' JSONのcards配列にだけfactIdと質問のオブジェクトを入れてください。candidatesは同姓同名の人物確認専用で通常は空配列です。cardsとcandidatesを混同せず、cardsは0件でも必ず含めてください。', config.orcaAssessmentModel);
      const cards = result.value.cards.flatMap(({ factId, displayFact, displayQuestion, ...card }) => {
        const selected = selectedFacts.get(factId);
        // Safety applies to the model's complete proposal before an invalid
        // or low-quality short helper can be discarded. Never hide a sensitive
        // suggested topic by removing only its display text.
        if (!selected || !isAllowedConversationTopic(selected.fact, card.suggestedQuestion,
          typeof displayQuestion === 'string' ? displayQuestion : undefined)) return [];
        ...
        if (display.displayQuestion && !isPoliteDisplayQuestion(display.displayQuestion)) delete display.displayQuestion;
        return selected && isUsefulConversationQuestion(card.suggestedQuestion) && isAllowedConversationTopic(selected.fact, card.suggestedQuestion, display.displayQuestion)
